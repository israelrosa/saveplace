import CustomButton from 'components/CustomButton';
import { StatusBar } from 'expo-status-bar';
import React from 'react';
import { View } from 'react-native';
import { useTheme } from 'styled-components';

import {
  ActionsContainer,
  Card,
  CardAvatar,
  CardBox,
  CardHero,
  CardsContainer,
  CardSection,
  CardSpan,
  Container,
  Header,
  HeaderImage,
  HeaderText,
  SubHeader,
  SubHeaderText,
} from './styles';

const QueueDetails: React.FC = () => {
  const theme = useTheme();

  return (
    <Container>
      <StatusBar backgroundColor={theme.colors.primary} style="light" />
      <Header>
        <HeaderImage
          source={{
            uri: 'data:image/png;base64,',
          }}
        />
        <HeaderText>Banco Original</HeaderText>
      </Header>
      <SubHeader>
        <SubHeaderText>Fila 1</SubHeaderText>
      </SubHeader>
      <CardsContainer>
        <View style={{ flexDirection: 'row' }}>
          <Card>
            <CardBox style={{ backgroundColor: theme.colors.background.grey }}>
              <CardSpan>Atual</CardSpan>
              <CardSection>Senha: 00002</CardSection>
            </CardBox>
            <CardBox style={{ flexDirection: 'row', alignItems: 'center' }}>
              <CardAvatar
                source={{
                  uri: 'data:image/png;base64,',
                }}
              />
              <CardSection style={{ marginLeft: 12 }}>Banco</CardSection>
            </CardBox>
          </Card>
          <View style={{ width: 12 }} />
          <Card>
            <CardBox style={{ backgroundColor: theme.colors.background.grey }}>
              <CardSpan>Próximo(a) da fila</CardSpan>
              <CardSection>Senha: 00002</CardSection>
            </CardBox>
            <CardBox style={{ flexDirection: 'row', alignItems: 'center' }}>
              <CardAvatar
                source={{
                  uri: 'data:image/png;base64,',
                }}
              />
              <CardSection style={{ marginLeft: 12 }}>Banco</CardSection>
            </CardBox>
          </Card>
        </View>
        <Card>
          <CardBox style={{ backgroundColor: theme.colors.background.grey }}>
            <CardSection>Sua senha</CardSection>
          </CardBox>
          <CardBox>
            <CardHero>00041</CardHero>
          </CardBox>
        </Card>
        <View style={{ flexDirection: 'row' }}>
          <Card>
            <CardBox>
              <CardHero>41</CardHero>
            </CardBox>
            <CardBox style={{ backgroundColor: theme.colors.background.grey }}>
              <CardSection>Posição</CardSection>
              <CardSpan>Na fila</CardSpan>
            </CardBox>
          </Card>
          <View style={{ width: 12 }} />
          <Card>
            <CardBox>
              <CardHero>5</CardHero>
            </CardBox>
            <CardBox style={{ backgroundColor: theme.colors.background.grey }}>
              <CardSection>Minutos</CardSection>
              <CardSpan>De espera</CardSpan>
            </CardBox>
          </Card>
        </View>
        <ActionsContainer>
          <CustomButton color={theme.colors.primary} text="Sair da Fila" />
        </ActionsContainer>
      </CardsContainer>
    </Container>
  );
};

export default QueueDetails;
